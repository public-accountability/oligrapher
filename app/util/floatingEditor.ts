...
export const set = (
  ...
  t: FloatingEditorTypeType | null = null,
  id: string | null = null
): void => {
...
export const getType = (display: DisplayState): FloatingEditorTypeType | null =>
  display.floatingEditor.type
...
export const floatingEditorPositionSelector = (state: State): Point | null => {
  ...
  const itemRect = getElementForGraphItem(
    floatingEditor.id,
    floatingEditor.type
  ...
  // use center for edges
  if (floatingEditor.type === "edge") {
    point.x = point.x - itemRect.width / 2
  }
