...
export function getNodeUIState(id: string, state: State): NodeUIState {
  return {
    ...
    edited: Boolean(
      state.display.floatingEditor.type === "node" && state.display.floatingEditor.id === id
    ),
    selected: state.display.selection.node.includes(id),
    highlighted: Boolean(
      state.display.modes.story &&
        state.annotations.list[state.annotations.currentIndex]?.nodeIds?.includes(id)
    ),
    appearance: calculateAppearance(id, state),
    // disable dragging if not editing and while another node is being dragged
    disabled:
      (!state.display.modes.editor && !state.attributes.settings.nodeDraggingWhenPresenting) ||
      Boolean(
        state.display.modes.editor && state.display.draggedNode && state.display.draggedNode !== id
      ),
  }
}
