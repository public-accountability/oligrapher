...
import Graph, { Graph as GraphType } from "../graph/graph"
...
*/
export default function AddConnections({ id }: { id: string }) {
  const dispatch = useDispatch()
  const graph = useSelector<State, GraphType>(state => state.graph)
  const { x, y } = useSelector<State>(state => state.graph.nodes[id])
  const connectedNodeIds = Graph.connectedNodeIds(graph, id)
  ...
  useEffect(() => {
    ...
    const httpRequest = makeCancelable(findConnections({ entity_id: id, category_id: categoryId }))
