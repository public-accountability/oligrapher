...
import { findConnections, LsNodeWithEdges, AddConnectionsOrder } from "../datasources/littlesis"
...
function littleSisNodesSelector(state: State): Array<string> {
  return Graph.littleSisNodes(state.graph).map(n => toString(n.id))
}

function createPositionSelector(id: string): (state: State) => Point {
...
export default function AddConnectionsClassic(props: { id: string }) {
  ...
  useEffect(() => {
    ...
    httpRequest.promise
      .then(results => {
        console.debug(
          `Find connections results for ${props.id} / ${categoryId} / ${littleSisIds}`,
          results
        )
        setResults(results)
        setLoading(false)
  ...
  return (
    <div className="oligrapher-add-connections">
      ...
      <Button
        className="add-connections-classic-button"
