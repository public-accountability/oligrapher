...
export default function FloatingEditors() {
  ...
  const useClassicAddConnections = useSelector(
    state => state.attributes.settings.useClassicAddConnections
  )
  ...
  return (
    <Draggable key={key} handle=".editor-header" positionOffset={position || undefined}>
      <div className="oligrapher-floating-editor" test-id="oligrapher-floating-editor">
        ...
        {floatingEditor.type === "connections" && !useClassicAddConnections && (
          <AddConnections id={id} />
        )}
        {floatingEditor.type === "connections" && useClassicAddConnections && (
          <AddConnectionsClassic id={id} />
        )}
